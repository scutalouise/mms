...
$(function(){
	...
	$("#right_panel").layout("add", {
		...
		title : "报警条件",
		split : true,
		href : ctx + "/system/deviceAlarmCondition"
	});
	$("#right_panel").layout("add", {
		title : "报警规则",
		region : "center",
		href : ctx + "/system/deviceAlarmRule"
	});
	...
	template_dg = $("#template_dg").datagrid({
		...
		onSelect:function(rowIndex, rowData){
			$("#condition_add").linkbutton('enable');   
			
			checkedTemplateId=rowData.id;
			checkedTemplateName=rowData.name; 
			condition_dg.datagrid("options").url=ctx + "/system/deviceAlarmCondition/json";
			condition_dg.datagrid("reload",{
				
				"filter_EQI_alarmTemplate.id" : rowData.id,
				filter_EQ_StatusEnum_status : 'NORMAL'
			});
		}
	});
...
function addTemplate() {
	...
	template_d = $("#template_dlg").dialog({
		...
		width : 400,
		height : 250,
		href : ctx + "/system/alarmTemplate/addForm/DEVICEALARM",
		maximizable : true,
		modal : true,
		buttons : [ {
			text : "确认",
