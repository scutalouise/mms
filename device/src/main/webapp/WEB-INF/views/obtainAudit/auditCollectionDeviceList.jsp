	...
	<script type="text/javascript">
		...
		function audit(type){
			...
			$.ajax({
				...
				url:"${ctx}/device/collectionDevice/collectionDeviceAudit/"+type,
		...
		function operation(type){
			...
			$.ajax({
				...
				data:JSON.stringify(collectionDeviceIdList),
				contentType:'application/json;charset=utf-8',	//必须
				url:"${ctx}/device/collectionDevice/"+type,
				success:function(data){
					if(data=='success'){
						parent.$.messager.show({ title : "提示",msg: "操作成功！", position: "bottomRight" });
						collection_dg.datagrid('reload'); 
			...
			parent.$.messager.confirm('提示', '确定要报废设备吗？', function(data){
				if (data){
					$.ajax({
						...
						data:JSON.stringify(collectionDeviceIdList),
						contentType:'application/json;charset=utf-8',	//必须
						url:"${ctx}/device/collectionDevice/scrap",
						success:function(data){
							if(data=='success'){
								parent.$.messager.show({ title : "提示",msg: "操作成功！", position: "bottomRight" });
								collection_dg.datagrid('reload'); 
