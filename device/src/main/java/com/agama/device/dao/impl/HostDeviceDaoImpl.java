...
public class HostDeviceDaoImpl extends HibernateDaoImpl<HostDevice, Integer>implements IHostDeviceDao {
	...
	public Long getInventoryCountBySecondDeviceType(Integer orgId,
		...
		StringBuffer hql=new StringBuffer("select count(*) from HostDevice where status='").append(StatusEnum.NORMAL.toString()).append("' and purchaseId in (select id from DevicePurchase where orgId=").append(orgId).append(" and deviceInventoryId in (select id from DeviceInventory where secondDeviceType='").append(secondType.toString()).append("'))");
