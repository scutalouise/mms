...
<body>
		...
		<form:form id="rule_mainform"
			...
			<table class="formTable" cellpadding="3"
				...
				<tr>
					...
					<td><form:hidden path="id" /> <form:hidden path="status"
							value="0" /> <form:hidden path="version" /> <form:hidden
							path="deviceInterfaceType" id="rule_deviceInterfaceType"/>
							<input id="rule_deviceInterfaceTypeText" class="easyui-validatebox">
							</td>
					<td width="80px">报警条件:</td>
					<td><form:input path="alarmConditionName"
							id="alarmConditionName" class="easyui-validatebox"/> <form:hidden id="alarmConditionId"
						path="alarmConditionId"/></td>
				</tr>
				<tr>
					<td>报警类型:</td>
					<td><form:select path="alarmType" id="alarmType"
							style="width: 135px;">
							<c:forEach items="${alarmTypes }" var="alarmType">
								<option value="${alarmType }">${alarmType.name }</option>
							</c:forEach>
						</form:select></td>
					<td>规则比较:</td>
				...
				<tr >
					<td>规则类型:</td>
					<td>
					<select class="easyui-combobox" name="ruleAlarmType" id="state" style="width:135px;">
				...
				<tr>
					...
					<td colspan="3"><form:textarea path="remark" 
							class="easyui-validatebox" style="height: 60px; width: 386px;" data-options="required:true,validType:['maxLength[50]']"></form:textarea></td>
				</tr>
			</table>
		</form:form>
	</div>
	...
	function dialogTop() {
		...
		$(function() {
			...
			$("#alarmConditionName").attr("readonly", "readonly");
			$("#alarmConditionName").css('background', '#eee')
			if (action == "update") {
				$("#state").combobox("setValue","${alarmRule.ruleAlarmType}");
