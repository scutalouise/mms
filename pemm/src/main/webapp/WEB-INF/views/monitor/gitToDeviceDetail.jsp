...
<script type="text/javascript">
	...
	function windowDeviceResize() {
		...
		$(".other_content_div").each(function(){
			var left=($(window).width()-45-$(this).width())/2;
			left=left>0?left:0;
	...
	function back(){
		window.location.href="${ctx}/upsStatus/gitInfoView";
	}
...
<body>
	<center>
	<div style="text-align: right;margin: 5px"><a class="btn btn-default" href="javascript:back();">返回</a></div>
		...
		<div id="connect">
			<c:forEach items="${gitInfoList }" var="gitInfo">
				<div
					style="font-size: 16px; font-weight: 800; margin-left: 10px; margin: 10px;text-align: left;">网点名称：【${gitInfo.name }】<a href="javascript:closeAlarm('${gitInfo.ip }')" class='btn btn-default'>关闭报警</a>&nbsp;&nbsp;</div>
					...
					<div style="text-align: left;">${upsStatus.deviceName }</div>
						<div class="ups_detail">
							...
							<div class="ups_detail_left">
								...
								<div class="batteryStatus">
									<table>
										<tr>
											<td align="left" style="padding-left: 20px;" >
											<c:if test="${upsStatus.upsType!=2&&upsStatus.upsType!=3}">
											...
											电池电压：<span id="batteryVoltage_${varStatus.index }">${upsStatus.batteryVoltage}</span>V<br>
											</c:if>
							...
							<div class="ups_detail_center">
								...
								<div class="remainingTime">剩余时间：<span id="remainingTime_${varStatus.index }">${upsStatus.remainingTime }</span>分钟</div>
							</div>
							...
							<div class="other_div">
								...
								<div class="other_content">
									...
									<div class="th_content_detail">
										...
										<br> 温度：<span id="temperature_${varThStatus.index }">
										...
										湿度：<span id="humidity_${varThStatus.index }">
				...
				<div class="ac_border" >
					...
					<c:forEach items="${gitInfo.acStatusList }" var="acStatus">
						<div>
						室内温度：${acStatus.indoorTemperature/10 }℃<br/>
						室内湿度：${acStatus.indoorHumidity/10 }%<br/>
						室外温度：${acStatus.outdoorTemperature }<br/>
						
						<a class="btn btn-default" href="javascript:acCloseOrOpen('${gitInfo.ip }','${acStatus.deviceIndex }',1)">关机</a>
						<a class="btn btn-default" href="javascript:acCloseOrOpen('${gitInfo.ip }','${acStatus.deviceIndex }',2)">开机</a>
						</div>
				...
				<div style="height: 120px; float: left;">&nbsp;</div>
