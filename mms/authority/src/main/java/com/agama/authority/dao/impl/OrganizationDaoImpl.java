...
public class OrganizationDaoImpl extends HibernateDaoImpl<Organization, Integer> implements IOrganizationDao{
	...
	@Override
	public List<Organization> findListByAreaIdAndStatus(Integer areaId,
			StateEnum state,String searchValue) {
		...
		return this.getSession().createSQLQuery(hql).setResultTransformer(Transformers.aliasToBean(Organization.class))
				.list();
	}
