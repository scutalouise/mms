...
/**
 * @Description:定时任务 service实现
 * @Author:scuta
 * @Since :2015年8月27日 上午10:41:27
 */
...
public class ScheduleJobServiceImpl implements IScheduleJobService{
	...
	public void add(ScheduleJob scheduleJob){
		...
		CronTrigger trigger = TriggerBuilder.newTrigger().withIdentity(scheduleJob.getName(), scheduleJob.getGroup()).withSchedule(scheduleBuilder).withDescription(scheduleJob.getDescription()).build();
	...
	public void modify(ScheduleJob scheduleJob) {
		try {  
            ...
            CronTrigger newTrigger = (CronTrigger) TriggerBuilder.newTrigger()  
                    .withIdentity(key)
                    .withSchedule(CronScheduleBuilder.cronSchedule(scheduleJob.getCronExpression())).withDescription(scheduleJob.getDescription())
                    .build();  
            scheduler.rescheduleJob(key, newTrigger);  
        } catch (SchedulerException e) {  
            e.printStackTrace();  
