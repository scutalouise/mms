...
(function($) {
	...
	var homePageTitle = "主页", homePageHref = null, navMenuList = "#navMenu_list", navMenuTree = "#navMenu_Tree", mainTabs = "#mainTabs", navTabs = "#navTabs", favoMenuTree = "#favoMenu_Tree", mainLayout = "#mainLayout", northPanel = "#northPanel", themeSelector = "#themeSelector", westLayout = "#westLayout", westCenterLayout = "#westCenterLayout", westFavoLayout = "#westFavoLayout", westSouthPanel = "#westSouthPanel", homePanel = "#homePanel", btnContact = "#btnContact", btnFullScreen = "#btnFullScreen", btnExit = "#btnExit", btnUpdatePwd = "#btnUpdatePwd", btnDeviceView = "#btnDeviceView", btnBackMain = "#btnBackMain", btnBackHome = "#btnBackHome", deviceBtnExit = "#deviceBtnExit";
	...
	window.mainpage.bindToolbarButtonEvent = function() {
		...
		var l = $(mainLayout), north = l.layout("panel", "north"), panel = north
				.panel("panel"), toolbar = $("#toolbar"), topbar = $("#topbar");
		opts = north.panel("options"), onCollapse = opts.onCollapse,
		...
		var themeName = $.cookie("themeName"), themeCombo = $(themeSelector)
				.combobox(
						{
							width : 100,
							editable : false,
							...
							textField : "name",
							value : themeName
									|| window.mainpage.themeData[0].path,
							onSelect : function(record) {
								var opts = themeCombo.combobox("options");
								window.mainpage.setTheme(
										record[opts.valueField], true);
							}
						});
				...
				.click(
						function() {
							if ($.util.supportsFullScreen) {
								...
								// $.easyui.messager.show("当前浏览器不支持全屏
								// API，请更换至最新的 Chrome/Firefox/Safari 浏览器或通过 F11
								// 快捷键进行操作。");
							}
						});
	...
	window.mainpage.search = function(value, name) {
		$.easyui.messager.show($.string.format("您设置的主题为：value: {0}, name: {1}",
				value, name));
	};

	window.mainpage.themeData = $.array.filter($.easyui.theme.dataSource,
			function(val) {
	...
	window.mainpage.setTheme = function(theme, setCookie) {
		...
		$.easyui.theme(true, theme, function(item) {
			if (setCookie) {
				$.cookie("themeName", theme, {
					expires : 30
				});
				var msg = $.string.format("您设置了新的系统主题皮肤为：{0}，{1}。", item.name,
						item.path);
	...
	window.mainpage.bindMainTabsButtonEvent = function() {
		...
		$("#mainTabs_refTab").click(function() {
			window.mainpage.mainTabs.refCurrentTab();
		});
	...
	window.mainpage.addModuleTab = function(node) {
		var n = node || {}, attrs = node.attributes || {}, opts = $.extend({},
				n, {
					title : n.text
				}, attrs);
		if (!opts.href) {
	...
	window.mainpage.mainTabs.isExists = function(title, href) {
		var t = $(mainTabs), tabs = t.tabs("tabs"), array = $.array.map(tabs,
				function(val) {
	...
	window.mainpage.mainTabs.createTab = function(title, href, iconCls,
			iniframe, closable, refreshable, selected) {
		var t = $(mainTabs), i = 0, opts = window.mainpage.mainTabs
				.parseCreateTabArgs(arguments);
	...
	// function (title, href, iconCls, iniframe, closable, refreshable,
	// selected)
	window.mainpage.mainTabs.addModule = function(title, href, iconCls,
			iniframe, closable, refreshable, selected) {
		var opts = window.mainpage.mainTabs.parseCreateTabArgs(arguments), isExists = window.mainpage.mainTabs
				.isExists(opts.title, opts.href);
		switch (isExists) {
		case 0:
	...
	window.mainpage.mainTabs.jumpHome = function() {
		var t = $(mainTabs), tabs = t.tabs("tabs"), panel = $.array.first(tabs,
				function(val) {
					var opts = val.panel("options");
					return opts.title == homePageTitle
							&& opts.href == homePageHref;
				});
