package com.agama.itam.snmp.trap;
		...
		if (command != null) {
			...
			for (int i = 0; i < recVBs.size(); i++) {
				...
				Integer index=Integer.parseInt(oid.substring(oid.lastIndexOf(".")+1));
					...
					List<PeDevice> peDeviceList = peDeviceService
							.getPeDeviceByIpAndDeviceTypeAndIndex(ip,
									DeviceType.SWITCHINPUT, index);
					for (PeDevice peDevice : peDeviceList) {
						//String status=recVB.getVariable().toString();
						switchInputStatusService.collectSwitchInputStatus(ip, index,peDevice.getIdentifier());
