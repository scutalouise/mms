...
<body>
		...
		<div data-options="region:'center'">
			<div id="tb" style="padding: 5px; height: auto">
				<form id="searchFrom" action="" method="post">
				    <label>网点名称:</label>
				    <input type="text" name="orgId" class="easyui-combobox" id="orgId" data-options="width:200"/>&nbsp;&nbsp; 
					<label>设备类型:</label>
					...
					<select name="sort" id="sort" class="easyui-combobox" data-options="width: 150">
						<option value="desc">高</option>
						<option value="asc">低</option> 
				    </select>&nbsp;&nbsp; 
				    <label>起始日期:</label>
				...
				$('#orgId').combotree({
					...
					onSelect:function(v){
						if(v.id==""){
							...
							$('#deviceType').combobox({ disabled: false });
							getDeviceType();
						}
					}
			    });
 ...
 function getDeviceType(){
	 $.ajax({
		    url:'${ctx}/device/brand/secondDeviceTypeList',
			type : "get",
			dataType : "json",
			success : function(data) {
				var json = {"secondDeviceType":"","name":"-- 请选择 --"};
				data.unshift(json);
			    $('#deviceType').combobox({
				   editable:false,
				   ...
				   onLoadSuccess:function(data){
				      $('#deviceType').combobox("setValue",data[0].secondDeviceType);
				   },
		       });
			}
	   });
 ...
 function initChart(){
	 require.config({
	    ...
	    $.ajax({
			async : false,
			type: 'post',
			url : "${ctx}/device/chart/deviceFaultTopNDataJson",
			data : {
			...
			success : function(json) {
				for (var i = 0; i < json.length; i++) {
					values[i] = json[i].quantity;
					if(orgId==""){
					   categories[i]=json[i].orgName;
					}else{
					   categories[i]=json[i].orgName+"【"+json[i].deviceType+"】";
					}
				}
				myChart.hideLoading();
			  }
